 ...
 * SET GPIO AS BELOW:
 *
 * PF10 INT  (input) T_PEN
 * PB2  MISO (input) T_MISO
 * PF9  MOSI (output)T_MOSI
 * PB1  SCLK (output)T_SCK
 * PF11  CS  (output)T_CS
 *
 * MUST IMPORT SYSTEM/delay.h
 */
_m_tp_dev tp_dev=
{
	TP_Init,
...
{  
	...
	{ 	  
		if(num&0x80)HAL_GPIO_WritePin(T_MOSI_GPIO_Port,T_MOSI_Pin,SET);
		else HAL_GPIO_WritePin(T_MOSI_GPIO_Port,T_MOSI_Pin,RESET);
		num<<=1;    
		HAL_GPIO_WritePin(T_SCK_GPIO_Port, T_SCK_Pin, RESET);
		delay_us(1);
		HAL_GPIO_WritePin(T_SCK_GPIO_Port, T_SCK_Pin, SET);		//上升沿有效
	}		 			    
} 		 
//SPI读数据 
//从触摸屏IC读取adc值
...
{ 	 
	uint8_t count=0;
	uint16_t Num=0;
	HAL_GPIO_WritePin(T_SCK_GPIO_Port, T_SCK_Pin, RESET);		//先拉低时钟
	HAL_GPIO_WritePin(T_MOSI_GPIO_Port,T_MOSI_Pin,RESET); 	//拉低数据线
	HAL_GPIO_WritePin(T_CS_GPIO_Port, T_CS_Pin, RESET); 		//选中触摸屏IC
	TP_Write_Byte(CMD);//发送命令字
	delay_us(6);//ADS7846的转换时间最长为6us
	HAL_GPIO_WritePin(T_SCK_GPIO_Port, T_SCK_Pin, RESET);
	delay_us(1);    	   
	HAL_GPIO_WritePin(T_SCK_GPIO_Port, T_SCK_Pin, SET);		//给1个时钟，清除BUSY
	delay_us(1);    
	HAL_GPIO_WritePin(T_SCK_GPIO_Port, T_SCK_Pin, RESET);
	for(count=0;count<16;count++)//读出16位数据,只有高12位有效 
	{ 				  
		Num<<=1; 	 
		HAL_GPIO_WritePin(T_SCK_GPIO_Port, T_SCK_Pin, RESET);	//下降沿有效
		delay_us(1);    
 		HAL_GPIO_WritePin(T_SCK_GPIO_Port, T_SCK_Pin, SET);
 		if(DOUT)Num++; 		 
	}  	
	Num>>=4;   	//只有高12位有效.
	HAL_GPIO_WritePin(T_CS_GPIO_Port, T_CS_Pin, SET);		//释放片选
	return(Num);   
}
//读取一个坐标值(x或者y)
...
//提示字符串
uint8_t TP_REMIND_MSG_TBL[]="Please use the stylus click the cross on the screen.The cross will always move until the screen adjustment is completed.";
			...
			{			   
				...
				case 4:	 //全部四个点已经得到
					...
					TP_Save_Adjdata();  
 					...
 					//char charA[100]={0};
 					printf("%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n",pos_temp[0][0],pos_temp[0][1],pos_temp[1][0],pos_temp[1][1],pos_temp[2][0],pos_temp[2][1],pos_temp[3][0],pos_temp[3][1]);
					return;//校正完成				 
			}
		}
		delay_ms(10);
